...
def test_application_use_cases_do_not_import_app_state_or_facade() -> None:
    for path in _APPLICATION_ROOT.rglob("*.py"):
        ...
        for node in ast.walk(tree):
            ...
            if node.module == "my_code.runtime.state":
                assert "AppState" not in {alias.name for alias in node.names}, path
