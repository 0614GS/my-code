...
def test_credential_store_round_trip_uses_owner_only_file(tmp_path: Path) -> None:
    ...
    store.save_api_key("sk-test-value")

    assert store.load_api_key() == "sk-test-value"
...
def test_environment_has_precedence_over_stored_key(tmp_path: Path) -> None:
    store = CredentialStore(tmp_path / ".credentials.json")
    store.save_api_key("stored")

    resolved = resolve_api_key(store, {"ANTHROPIC_API_KEY": "temporary"})

    assert resolved.api_key == "temporary"
