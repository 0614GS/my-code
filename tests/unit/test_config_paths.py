...
from my_code.config.paths import MyCodePaths, SettingsScope, sanitize_path
...
def test_default_layout_matches_claude_code_shape(tmp_path: Path) -> None:
    ...
    paths = MyCodePaths.discover(workspace, environ={}, home=home)

    assert paths.config_home == home / ".my-code"
    ...
    assert paths.credentials_path == home / ".my-code" / ".credentials.json"
    assert paths.providers_path == home / ".my-code" / "providers.json"
    assert paths.settings_path(SettingsScope.PROJECT) == (
        workspace / ".my-code" / "settings.json"
    )
    assert paths.settings_path(SettingsScope.LOCAL) == (
        workspace / ".my-code" / "settings.local.json"
    )
...
def test_config_home_can_be_overridden(tmp_path: Path) -> None:
    ...
    paths = MyCodePaths.discover(
        workspace,
        environ={"MY_CODE_CONFIG_DIR": str(override)},
    )
