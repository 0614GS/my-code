...
| R8 | resume 使用当前 provider、permission 和 workspace，不从 Transcript 覆盖运行时配置 | 阶段 6 的 Chat 状态与 resume 投影测试覆盖 |

## Session 切换

| ID | 必须验证的行为 | 当前证据或落地阶段 |
| --- | --- | --- |
| S1 | 先完整构造候选 Session，再一次替换活动引用 | 阶段 6 的 `ChatService` bundle 成功/失败测试覆盖 |
| S2 | Session、Conversation、ContextSession、工具结果目录和展示索引一起切换 | 阶段 6 验证新 ContextSession 与目标 ToolResultStore |
| S3 | submit、stream、compact 与 resume 互斥，流式 turn 中不能切换 | 阶段 6 并发测试验证 resume 等待流式 turn 完成 |
| S4 | 切换失败后 session ID、history、context delivery 和工具结果 binding 均保持原值 | 阶段 6 空目标失败测试验证旧 bundle 对象不变 |
| S5 | TUI/CLI 只接收 Chat 投影，不持有核心消息，也不读取 JSONL | 阶段 6 AST 依赖与技术泄漏守卫覆盖 |
