...
| R7 | provider binding 匹配才恢复 continuation；不匹配时保留消息但丢弃 continuation | 待阶段 2/3 |
...
| I3 | request attachment、预算、microcompact proposal 和 provider 流状态在调用后丢弃 | 待阶段 5；provider 流状态在阶段 2 验证 |
