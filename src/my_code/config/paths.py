...
class MyCodePaths:
    ...
    def discover(
        ...
        *,
        ...
        """解析工作区和形如 ``~/.my-code`` 的配置主目录。"""

        environment = os.environ if environ is None else environ
        canonical_cwd = _canonical_path(cwd)
        config_override = environment.get("MY_CODE_CONFIG_DIR")
        if config_override:
            config_home = _canonical_path(Path(config_override).expanduser())
        else:
            user_home = Path.home() if home is None else home
            config_home = _canonical_path(user_home / ".my-code")
    ...
    def project_config_dir(self) -> Path:
        return self.cwd / ".my-code"

    @property
    def project_config_collides_with_user_storage(self) -> bool:
        """项目设置是否会占用用户配置目录。

        my-code 在用户主目录中以默认布局启动时会发生这种情况。同一个
